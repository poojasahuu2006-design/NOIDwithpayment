...
MYSQL_CONFIG = {
    ...
    "password": os.getenv("MYSQL_PASSWORD", "pooja@2006"),  # <-- your password
    "database": os.getenv("MYSQL_DATABASE", "noid_db"),
    "port": int(os.getenv("MYSQL_PORT", "3306")),    # MySQL usually runs on 3306
}
...
def ensure_database_and_tables() -> None:
    ...
    try:
        ...
        # Ensure pid column is VARCHAR for alphanumeric IDs (migrate if older INT)
        try:
            cur2.execute(
                """
            ...
            if not row or str(row[0]).lower() in ("int", "integer", "bigint"):
                # Force alter to VARCHAR for alphanumeric PIDs
                cur2.execute("ALTER TABLE students MODIFY pid VARCHAR(32) NOT NULL")
        except mysql.connector.Error:
            # Ignore if alter not needed or fails due to same definition
            pass

        # Ensure DOB column exists (for showing date of birth)
        try:
            cur2.execute(
                """
        ...
        # Seed with provided students (idempotent). If table empty, insert all.
        # If not empty, upsert to ensure these four exist/are updated.
        seed_students = [
...
def fetch_student_by_pid(pid: str) -> Optional[Dict[str, str]]:
    ...
    if row is None:
        return None
    # Convert all values to string for safe rendering
    result = {k: str(v) if v is not None else "" for k, v in row.items()}
    # Build a web URL for the photo inside static/photos/
    photo_value = result.get("photo_url", "")
    if photo_value and not photo_value.startswith("http"):
        # Accept either bare filename or path like photos/xyz.jpg
